...
int interval=80;
bool Nes_controller_I2C = true;
bool bouton_start=false;
long previous_millis_bouton_start=0;
bool bouton_up=false;
long previous_millis_bouton_up=0;
bool bouton_down=false;
long previous_millis_bouton_down=0;
bool bouton_left=false;
long previous_millis_bouton_left=0;
bool bouton_right=false;
long previous_millis_bouton_right=0;
bool bouton_a=false;
long previous_millis_bouton_a=0;
bool bouton_b=false;
long previous_millis_bouton_b=0;
void IRAM_ATTR ir_event(uint8_t ticks, uint8_t value); // t is HSYNCH ticks, v is value

void gotStart(){
 previous_millis_bouton_start=millis();
// printf("bouton start");
 bouton_start = true;
}
void gotUp(){
 bouton_up = true;
 previous_millis_bouton_up=millis();
}
void gotDown(){
 bouton_down = true;
 previous_millis_bouton_down=millis();
}
void gotLeft(){
 bouton_left = true;
 previous_millis_bouton_left=millis();
}
void gotRight(){
 bouton_right = true;
 previous_millis_bouton_right=millis();
}
void gotA(){
 bouton_a = true;
 previous_millis_bouton_a=millis();
}
void gotB(){
 bouton_b = true;
 previous_millis_bouton_b=millis();
}
...
int get_hid_buttons(uint8_t* dst)
{
 ...
 // Touches sensitives
     	buttonsA |= (bouton_a == true) * GENERIC_FIRE_A;
    	buttonsA |= (bouton_b == true)  * GENERIC_FIRE_B;
    	buttonsA |= (bouton_start == true) * GENERIC_START;
    	buttonsA |= (bouton_up == true) * GENERIC_UP;
    	buttonsA |= (bouton_down == true) * GENERIC_DOWN;
    	buttonsA |= (bouton_left == true) * GENERIC_LEFT;
    	buttonsA |= (bouton_right == true) * GENERIC_RIGHT;
      if(bouton_start){
      printf("start");
      }
      if(bouton_right && bouton_start){
      buttonsA |= GENERIC_OTHER;
      }  

    if(millis()-previous_millis_bouton_start>interval){  
        bouton_start = false;
        
      }
    if(millis()-previous_millis_bouton_up>interval){
        bouton_up = false;      
      }
    if(millis()-previous_millis_bouton_down>interval){       
        bouton_down = false;
      }
    if(millis()-previous_millis_bouton_left>interval){       
        bouton_left = false;
      }
    if(millis()-previous_millis_bouton_right>interval){       
         bouton_right = false;
      }
    if(millis()-previous_millis_bouton_a>interval){      
        bouton_a = false;
      }
    if(millis()-previous_millis_bouton_b>interval){       
        bouton_b = false;
      }
   if(Nes_controller_I2C){
      nes.update();   
     	buttonsA |= (nes.buttonA() == true) * GENERIC_FIRE_A;
    	buttonsA |= (nes.buttonB() == true)  * GENERIC_FIRE_B;
    	buttonsA |= (nes.buttonStart() == true) * GENERIC_START;
    	buttonsA |= (nes.dpadUp() == true) * GENERIC_UP;
    	buttonsA |= (nes.dpadDown() == true) * GENERIC_DOWN;
    	buttonsA |= (nes.dpadLeft() == true) * GENERIC_LEFT;
    	buttonsA |= (nes.dpadRight() == true) * GENERIC_RIGHT;        
    	buttonsA |= (nes.buttonSelect() == true) * GENERIC_SELECT;
