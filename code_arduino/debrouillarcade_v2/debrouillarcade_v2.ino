...
void setup()
{
  ...
  touchAttachInterrupt(33, gotStart, threshold);
  touchAttachInterrupt(14, gotDown, threshold);
  touchAttachInterrupt(27, gotLeft, threshold);
  touchAttachInterrupt(12, gotUp, threshold);
  touchAttachInterrupt(13, gotB, threshold);
  touchAttachInterrupt(32, gotRight, threshold);
  touchAttachInterrupt(15, gotA, threshold);
  // setup momentary contact buttons as the controller
  nes.begin();

  if (!nes.connect()) {
    printf("NES Classic Controller not detected!");
    Nes_controller_I2C=false;
    delay(1000);
  } else {
    printf("NES Classic Controller  detected!");
